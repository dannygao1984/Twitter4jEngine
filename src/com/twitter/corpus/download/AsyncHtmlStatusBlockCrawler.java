...
public class AsyncHtmlStatusBlockCrawler {
  ...
  public static String getUrl(long id, String username) {
    ...
    //System.out.println(String.format("http://twitter.com/%s/status/%d", username, id));
    //return String.format("http://twitter.com/%s/status/%d", username, id);
    
    ////http://twitter.com/#!/appinn/followers
    //http://twitter.com/?_escaped_fragment_=/RobOusbey
    String strUrl = String.format("http://twitter.com/#!/%s/followers", username);
...
//	Properties prop = twitterOAuth.getProp(); 
//	ConsumerKey consumer = new ConsumerKey(prop.getProperty("oauth.consumerKey"),
//	prop.getProperty("oauth.consumerSecret"));
//	com.ning.http.client.oauth.RequestToken user    = new com.ning.http.client.oauth.RequestToken(prop.getProperty("oauth.accessToken"), 
...
//	calc = new OAuthSignatureCalculator(consumer, user);
    ...
    try {
      ...
      while ((line = data.readLine()) != null) {
        ...
        asyncHttpClient.prepareGet(url)
        					.execute(new TweetFetcherHandler(id, username, url, false));
  ...
  private class TweetFetcherHandler extends AsyncCompletionHandler<Response> {
    ...
    public Response onCompleted(Response response) throws Exception {
      ...
      LogHandle.LogWriteline(id + ":" + username);      
      LogHandle.LogWriteline(url);      
      LogHandle.LogWriteline(response.getResponseBody("UTF-8"));
  				...
  				throws Exception 
  {
	  	...
	  	args = "-data block/20110206/20110206-000.dat -output html/20110206-000.html.seq".split(" ");
